...
\begin{itemize}
    \item \textbf{Phase 1} : Modélisation de base (machine seule) sur MATLAB et PSIM
    \item \textbf{Phase 2} : Intégration de la charge mécanique et des frottements
    \item \textbf{Phase 3} : Modélisation complète avec hacheur et validation croisée
    \item \textbf{Phase 4} : Conception et réglage des asservissements (courant et vitesse)
    \item \textbf{Phase 5} : Dimensionnement des composants et rapport intermédiaire
    \item \textbf{Phase 6} : Conception et réalisation du circuit imprimé (PCB)
    \item \textbf{Phase 7} : Modélisation 3D, assemblage et ajustements du système
    \item \textbf{Phase 8} : Tests expérimentaux et corrections
    \item \textbf{Phase 9} : Finalisation du projet, rapport final et soutenance
\end{itemize}
...
Ce rapport présente de manière détaillée les travaux réalisés et les résultats obtenus dans le cadre du projet final. Il s'articule autour des axes suivants :
\begin{itemize}
    \item Modélisation et validation des modèles de la MCC (Simulink / PSIM)
    \item Comparaison des outils de simulation et cohérence des résultats
    \item Conception et réglage des correcteurs pour les boucles courant et vitesse
    \item Dimensionnement des composants et conception du PCB
    \item Tests expérimentaux, validation sur maquette et analyse critique des performances
\end{itemize}

Les modèles et asservissements développés ont été validés par simulation et par essais sur maquette. Les performances mesurées satisfont les spécifications fixées en termes de régulation et de dépassement, et les choix de conception (composants, PCB) sont documentés et justifiés dans les chapitres dédiés.