...
\begin{equation*}
f = C_f \cdot \Omega = \frac{30}{\pi} \cdot \frac{1}{1000} \cdot 0{,}53 \cdot 10^{-2} = 5{,}06 \cdot 10^{-5} \, N.m/rad.s
\end{equation*}
...
\begin{tcolorbox}[colback=remindercolor!5!white, colframe=remindercolor!75!black, title=Formules d'une MCC avec pertes]
\begin{equation*}
u(t) = R \cdot i(t) + L \frac{di(t)}{dt} + e(t)
\end{equation*}
...
\begin{equation*}
J \frac{d\omega}{dt} = C_m(t) - f \cdot \omega(t) - C_r(t)
\end{equation*}
\end{tcolorbox}
...
\begin{equation*}
\Omega(s) = \frac{C_m + C_r}{Js + f}
\end{equation*}
\end{tcolorbox}

Une fois les équations trouvées, nous allons pouvoir déterminer le schéma bloc et effectuer les mesures.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{images/Moteur_Charge_Frottement/Schéma_PSIM_Moteur_Charge_Frottement.png}
    \caption{Modélisation du moteur avec charge et frottements sur PSIM}
    \label{fig:schema_moteur_charge_psim}
\end{figure}
\figref{Figure \ref{fig:schema_moteur_charge_psim}}{Modelisation/02\_22\_09\_2025/moteur\_plus\_gene\_plus\_charge\_plus\_frottement\_22\_09\_2025.psimsch}
...
\end{figure}
\figref{Figure \ref{fig:comparaison_resultats_moteur_charge}}{Modelisation/02\_22\_09\_2025/comparasion\_moteur\_plus\_gene\_plus\_charge\_plus\_frottement\_22\_09\_2025.m}

\colorbox{yellow}{\textbf{TODO: Ajouter une observation ici}}
...
La commande PWM est générée en comparant, au moyen d'un comparateur (ou d'un AOP configuré), une tension continue de référence $V_e = V_{\text{commande}}$ avec un signal triangulaire $V_{\text{triangle}}$ qui varie typiquement de 0 à 15\,V. Le comparateur produit alors le signal $V_{\text{pwm}}$ dont le rapport cyclique est proportionnel à $V_{\text{commande}}$. En sortie du comparateur, des suiveurs (buffers) sont placés pour assurer une adaptation d'impédance et piloter correctement les étages de commande des MOSFET.

Ce modèle permet d'étudier le comportement statique et dynamique du hacheur (effet du filtrage, des commutations, des temps morts, etc.) et sert de base pour obtenir les courbes de comportement et les signaux de commande utilisés dans les simulations ultérieures.
...
\end{figure}
\figref{Figure \ref{fig:schema_moteur_hacheur_simulink}}{Modelisation/03\_29\_09\_2025/moteur\_hacheur\_29\_09\_2025.slx}
...
\end{figure}
\figref{Figure \ref{fig:comparaison_resultats_moteur_hacheur}}{Modelisation/03\_29\_09\_2025/comparaison\_moteur\_hacheur\_29\_09\_2025.m}
\colorbox{yellow}{\textbf{TODO: Ajouter une observation ici}}