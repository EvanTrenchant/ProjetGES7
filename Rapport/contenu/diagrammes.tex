...
\vspace{0pt}
A ce stade de l'analyse, nous disposons du schéma bloc complet de notre système, avant asservissement en vitesse. Comme illustré dans la Figure \ref{fig:schema_bloc_asservissement}, ce schéma est assez complexe en raison de la présence de plusieurs boucles de rétroaction imbriquées. Pour faciliter l'analyse et la conception des contrôleurs, il est nécessaire de simplifier ce schéma bloc en combinant les différents éléments.
\end{minipage}
\hfill
\begin{minipage}[t]{0.48\textwidth}
\vspace{0pt}
Cette simplification permet d'obtenir une représentation plus concise du système, facilitant ainsi l'analyse et la conception des contrôleurs.
\[
\begin{aligned}
\left\{
\begin{array}{l}
...
\begin{figure}[H]
    \centering
    \begin{tikzpicture}[auto, node distance=2cm,>=latex', scale=0.65, transform shape]
        ...
        \node [gain, right of=input, node distance=1.5cm] (commande) {$6.4$};
        \node [sum, right of=commande, node distance=1.5cm] (sum2) {};
        ...
        \draw [-] (integ) -- (omega);
        \draw [->] (omega) -- node[above] {$\delta \Omega$} (output);
        ...
        \node [green!60!black, font=\Large\bfseries, left of=retourA, node distance=2.2cm, xshift=-0.05cm, yshift=0.5cm] {C};
...
\[
\delta_{i_{M}} (s) = \dfrac{\dfrac{1}{R+L\cdot s}}{1+\dfrac{\left(K_\phi\right)^2\cdot H}{R+L\cdot s}} \cdot 6{,}4 \cdot \delta u_{commande} (s) = FTBO (s) \cdot 6{,}4 \cdot \delta u_{commande} (s)
\]

Pour s'assurer que notre fonction de transfert est correcte, il faudrait la valider par des simulations numériques en comparant les résultats obtenus. Il serait alors possible de poursuivre l'analyse en utilisant cette fonction de transfert simplifiée pour concevoir et optimiser le contrôleur de l'asservissement en vitesse du moteur.