...
À l'aide de l'outil PID Tuner de Simulink, nous obtenons un jeu de paramètres qui répondent au cahier des charges pour le correcteur PI :
\begin{tcolorbox}[colback=conclusioncolor!5!white, colframe=conclusioncolor!75!black, boxrule=0.5pt, arc=2mm]
    \centering
    Gain proportionnel : $K_p = 5{,}053$ - Gain intégral : $K_i = 36 \, 349{,}6$
\end{tcolorbox}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{images/BF_Courant/Schéma_Simulink_Asservissement_Courant.png}
    \caption{Schéma complet de l'asservissement en courant du moteur dans Simulink}
    \label{fig:asservissement_courant_simulink}
\end{figure}
\figref{Figure \ref{fig:asservissement_courant_simulink}}{Modélisation\_Finale\_VALIDE\_13\_01\_2026/03\_BF\_Courant/BF\_Courant\_VALIDE\_06\_11\_2025.slx}
...
Dans Simulink : 
\[ G(s) = K_p + \dfrac{K_i}{s} = K_p \dfrac{s + \tfrac{K_i}{K_p}}{s} = K_p \dfrac{1 + \tfrac{K_p}{K_i} \cdot s}{\tfrac{K_p}{K_i} \cdot s} \]

Dans PSIM : 
\[ G(s) = k \dfrac{1 + T \cdot s}{T \cdot s} \]
...
\begin{figure}[H]
    ...
    \caption{Schéma complet de l'asservissement en courant du moteur dans PSIM}
    \label{fig:asservissement_courant_psim}
\end{figure}
\figref{Figure \ref{fig:asservissement_courant_psim}}{Modélisation\_Finale\_VALIDE\_13\_01\_2026/03\_BF\_Courant/BF\_Courant\_VALIDE\_06\_11\_2025.psimsch}
...
\end{figure}
\figref{Figure \ref{fig:comparaison_reponse_indicielle_courant}}{Modélisation\_Finale\_VALIDE\_13\_01\_2026/03\_BF\_Courant/Comparaison\_Asservissement\_Courant\_VALIDE\_06\_11\_2025.m}
...
Dans cette section, nous utilisons une dynamo tachymétrique comme capteur de vitesse. Ce capteur analogique fournit une tension proportionnelle à la vitesse de rotation, ce qui permet une mise en œuvre simple et une mesure continue de la vitesse.

\subsection{Principe de fonctionnement de la dynamo tachymétrique}

La dynamo tachymétrique est une petite génératrice à courant continu couplée mécaniquement au moteur. Elle génère une tension proportionnelle à sa vitesse de rotation selon la relation :
\begin{equation*}
    V_{\text{tachy}} = K_{\text{tachy}} \cdot \Omega
\end{equation*}
où $K_{\text{tachy}}$ est la constante de la dynamo tachymétrique (en V/(rad/s)) et $\Omega$ la vitesse angulaire du moteur.

Cette caractéristique linéaire en fait un capteur particulièrement adapté pour les asservissements analogiques, car la tension de sortie peut être directement utilisée comme signal de rétroaction sans nécessiter de conversion numérique-analogique.
...
\end{figure}
\figref{Figure \ref{fig:schéma_BO_vitesse_simulink}}{Modélisation\_Finale\_VALIDE\_13\_01\_2026/04\_BF\_Courant\_Vitesse/BO\_Vitesse/BO\_Vitesse\_VALIDE\_06\_11\_2025.slx}
...
L'analyse de la réponse indicielle révèle un temps de réponse à \SI{95}{\percent} en boucle ouverte de \SI{322,1}{ms}. Cette valeur relativement élevée indique une dynamique lente qu'il faudra améliorer.
...
\end{figure}
\figref{Figure \ref{fig:asservissement_vitesse_simulink}}{Modélisation\_Finale\_VALIDE\_13\_01\_2026/04\_BF\_Courant\_Vitesse/BO\_Vitesse/BO\_Vitesse\_VALIDE\_06\_11\_2025.slx}

\subsection{Conception du correcteur PI}

Face aux limitations observées en boucle ouverte, nous mettons en œuvre un asservissement en boucle fermée avec un correcteur PI. L'objectif de cet asservissement est d'améliorer drastiquement les performances dynamiques du système. Nous visons un temps de réponse à \SI{5}{\percent} trois fois plus rapide qu'en boucle ouverte, soit \SI{107,37}{ms}, tout en maintenant un dépassement maximal de \SI{20}{\percent} pour limiter les contraintes mécaniques.
...
\end{figure}
\figref{Figure \ref{fig:asservissement_vitesse_simulink_schema}}{Modélisation\_Finale\_VALIDE\_13\_01\_2026/04\_BF\_Courant\_Vitesse/BF\_Vitesse/BF\_Courant\_Vitesse\_VALIDE\_06\_11\_2025.slx}
...
\begin{tcolorbox}[colback=conclusioncolor!5!white, colframe=conclusioncolor!75!black, boxrule=0.5pt, arc=2mm]
\begin{itemize}
    \item Gain proportionnel : $K_p = 2{,}00$
    \item Gain intégral : $K_i = 78{,}86$
\end{itemize}
\end{tcolorbox}
...
\end{figure}
\figref{Figure \ref{fig:asservissement_vitesse_psim_schema}}{Modélisation\_Finale\_VALIDE\_13\_01\_2026/04\_BF\_Courant\_Vitesse/BF\_Vitesse/BF\_Asservissement\_Courant\_Vitesse\_VALIDE\_06\_11\_2025.psimsch}
...
\end{figure}
\figref{Figure \ref{fig:comparaison_reponses_vitesse}}{Modélisation\_Finale\_VALIDE\_13\_01\_2026/04\_BF\_Courant\_Vitesse/Comparaison\_BF\_Vitesse/Comparaison\_Asservissement\_Courant\_Vitesse\_VALIDE\_06\_11\_2025.m}
...
\begin{itemize}
    \item Le temps de réponse à \SI{5}{\percent} est d'environ \SI{106}{ms}, soit plus de trois fois plus rapide que la réponse en boucle ouverte (\SI{107,37}{ms})
    \item Le dépassement observé reste inférieur à \SI{20}{\percent} ( \SI{17,8}{\percent}), ce qui limite les contraintes mécaniques sur le système d'entraînement
    \item Le régime permanent est atteint sans erreur statique grâce à l'action intégrale du correcteur PI
\end{itemize}

\section{Asservissement en vitesse avec codeur incrémental}

Le cahier des charges impose également la possibilité d'asservir la vitesse du moteur à l'aide d'un codeur incrémental. Contrairement à la dynamo tachymétrique qui fournit un signal analogique continu, le codeur incrémental est un capteur numérique qui génère des impulsions proportionnelles à la vitesse de rotation. Ce type de capteur offre une précision élevée et une meilleure immunité au bruit électromagnétique que les capteurs analogiques.

Dans cette section, nous présentons d'abord le principe de fonctionnement du codeur incrémental, puis nous validons son utilisation dans l'asservissement en vitesse à travers des simulations avec des correcteurs PI idéaux sur Simulink et PSIM.

\subsection{Principe de fonctionnement du codeur incrémental}
...
Le codeur incrémental fourni est composé de deux pistes de fentes décalées d'un quart de période, ce qui permet de déterminer le sens de rotation du moteur en analysant la séquence des impulsions générées par les deux capteurs.

\subsection{Calcul de la vitesse et du sens de rotation}
...
\subsection{Simulation avec correcteurs PI idéaux sur PSIM}

A présent, nous implémentons l'asservissement en vitesse avec le codeur incrémental en utilisant des correcteurs PI idéaux (blocs PI de PSIM). Cette étape permet de valider le principe de fonctionnement du système avant de passer à l'implémentation avec des composants réels. Nous obtenons le schéma suivant pour la partie codeur incrémental :
\newpage
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{images/Codeur Incremental/Schéma_PSIM_Codeur_Incremental.png}
    \caption{Schéma de la partie codeur incrémental dans PSIM}
    \label{fig:schéma_codeur_incremental_psim}
\end{figure}
\figref{Figure \ref{fig:schéma_codeur_incremental_psim}}{Modélisation\_Finale\_VALIDE\_13\_01\_2026/05\_Codeur\_Incrémentale/BF\_Vitesse\_Codeur\_Incrementale\_VALIDE\_08\_11\_25.psimsch}

Le schéma complet de l'asservissement en vitesse avec codeur incrémental devient ainsi :

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{images/Codeur Incremental/Schéma_PSIM_BF_Vitesse_Plus_Codeur_Incremental.png}
    \caption{Schéma de l'asservissement en vitesse avec codeur incrémental}
    \label{fig:schéma_asservissement_vitesse_codeur_incremental}
\end{figure}
\figref{Figure \ref{fig:schéma_asservissement_vitesse_codeur_incremental}}{Modélisation\_Finale\_VALIDE\_13\_01\_2026/05\_Codeur\_Incrémentale/BF\_Vitesse\_Codeur\_Incrementale\_VALIDE\_08\_11\_25.psimsch}

\subsection{Comparaison des résultats avec codeur incrémental}

Nous comparons ici les résultats obtenus avec Simulink sans le codeur incrémental et PSIM avec l'asservissement en vitesse utilisant le codeur incrémental. Cette comparaison permet de valider la cohérence de nos modélisations et de vérifier que les performances sont conformes au cahier des charges.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{images/Codeur Incremental/Graphe_Comparaison_BF_Vitesse_Codeur_Incremental.png}
    \caption{Comparaison des résultats de simulation entre Simulink et PSIM avec codeur incrémental}
    \label{fig:comparaison_simulink_psim}
\end{figure}
\figref{Figure \ref{fig:comparaison_simulink_psim}}{Modélisation\_Finale\_VALIDE\_13\_01\_2026/05\_Codeur\_Incrémentale/Comparaison\_Vitesse\_Codeur\_Incremental\_VALIDE\_08\_11\_2025.m}

La figure \ref{fig:comparaison_simulink_psim} présente une superposition des réponses en vitesse obtenues avec l'asservissement utilisant la dynamo tachymétrique sur Simulink et celui utilisant le codeur incrémental sur PSIM. Les deux courbes présentent une concordance satisfaisante, validant ainsi la cohérence entre les deux approches de mesure de vitesse.

On observe toutefois une différence notable dans l'allure des signaux : la réponse avec la dynamo tachymétrique présente une courbe lisse, tandis que celle avec le codeur incrémental affiche des oscillations caractéristiques. Cette différence s'explique par la nature fondamentalement différente des deux capteurs :

\begin{itemize}
    \item \textbf{Dynamo tachymétrique} : capteur analogique générant une tension continue proportionnelle à la vitesse instantanée, produisant un signal de rétroaction lisse.
    
    \item \textbf{Codeur incrémental} : capteur numérique générant des impulsions discrètes. La vitesse est calculée en comptant les impulsions sur une fenêtre temporelle, introduisant une quantification qui crée un effet d'escalier entre deux impulsions successives.
\end{itemize}

Ces oscillations sont intrinsèques à la mesure numérique et leur amplitude dépend de la résolution du codeur (1800 impulsions/tour, soit \SI{0,36}{\degree}) et de la période d'échantillonnage.

Malgré ces oscillations, les performances dynamiques moyennes restent conformes au cahier des charges : temps de réponse à \SI{5}{\percent} d'environ \SI{106}{ms}, dépassement inférieur à \SI{20}{\percent}, et absence d'erreur statique. Le codeur incrémental constitue donc une alternative viable à la dynamo tachymétrique, offrant une meilleure immunité au bruit et une précision accrue. Les oscillations peuvent être atténuées par filtrage numérique si nécessaire.
