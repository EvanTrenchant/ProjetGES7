...
data_psim = readmatrix('Data_Essai_PSIM_Tachy.txt');
...
%On filtre les données pour ne prendre que quelques périodes
filtre = t_synchro <= 3;
t_maquette_coupe = t_synchro(filtre);
Consigne_maquette_coupe = Consigne_maquette(filtre);
Vitesse_maquette_coupe = Vitesse_maquette(filtre);
Courant_maquette_coupe = Courant_maquette(filtre);
